//===---- BLUCPUAsmParser.cpp - Parse BLUCPU assembly to MCInst instructions ----===//
//
...
#include "MCTargetDesc/BLUCPUMCELFStreamer.h"
...
#define DEBUG_TYPE "blucpu-asm-parser"

using namespace llvm;

namespace {
/// Parses BLUCPU assembly from a stream.
class BLUCPUAsmParser : public MCTargetAsmParser {
  const MCSubtargetInfo &STI;
  MCAsmParser &Parser;
  const MCRegisterInfo *MRI;
  const std::string GENERATE_STUBS = "gs";

  enum BLUCPUMatchResultTy {
    Match_InvalidRegisterOnTiny = FIRST_TARGET_MATCH_RESULT_TY + 1,
  };

#define GET_ASSEMBLER_HEADER
#include "BLUCPUGenAsmMatcher.inc"
  ...
  unsigned toDREG(unsigned Reg, unsigned From = BLUCPU::sub_lo) {
    MCRegisterClass const *Class = &BLUCPUMCRegisterClasses[BLUCPU::DREGSRegClassID];
...
public:
  BLUCPUAsmParser(const MCSubtargetInfo &STI, MCAsmParser &Parser,
...
/// An parsed BLUCPU assembly operand.
class BLUCPUOperand : public MCParsedAsmOperand {
  typedef MCParsedAsmOperand Base;
  enum KindTy { k_Immediate, k_Register, k_Token, k_Memri } Kind;

public:
  BLUCPUOperand(StringRef Tok, SMLoc const &S)
      : Kind(k_Token), Tok(Tok), Start(S), End(S) {}
  BLUCPUOperand(unsigned Reg, SMLoc const &S, SMLoc const &E)
      : Kind(k_Register), RegImm({Reg, nullptr}), Start(S), End(E) {}
  BLUCPUOperand(MCExpr const *Imm, SMLoc const &S, SMLoc const &E)
      : Kind(k_Immediate), RegImm({0, Imm}), Start(S), End(E) {}
  BLUCPUOperand(unsigned Reg, MCExpr const *Imm, SMLoc const &S, SMLoc const &E)
...
public:
  ...
  static std::unique_ptr<BLUCPUOperand> CreateToken(StringRef Str, SMLoc S) {
    return std::make_unique<BLUCPUOperand>(Str, S);
  }

  static std::unique_ptr<BLUCPUOperand> CreateReg(unsigned RegNum, SMLoc S,
                                               SMLoc E) {
    return std::make_unique<BLUCPUOperand>(RegNum, S, E);
  }

  static std::unique_ptr<BLUCPUOperand> CreateImm(const MCExpr *Val, SMLoc S,
                                               SMLoc E) {
    return std::make_unique<BLUCPUOperand>(Val, S, E);
  }

  static std::unique_ptr<BLUCPUOperand>
  CreateMemri(unsigned RegNum, const MCExpr *Val, SMLoc S, SMLoc E) {
    return std::make_unique<BLUCPUOperand>(RegNum, Val, S, E);
  }
...
bool BLUCPUAsmParser::invalidOperand(SMLoc const &Loc,
  ...
  if (ErrorInfo != ~0U) {
    if (ErrorInfo >= Operands.size()) {
      ...
      BLUCPUOperand const &Op = (BLUCPUOperand const &)*Operands[ErrorInfo];
...
bool BLUCPUAsmParser::missingFeature(llvm::SMLoc const &Loc,
                                  uint64_t const &ErrorInfo) {
  return Error(Loc, "instruction requires a CPU feature not currently enabled");
}

bool BLUCPUAsmParser::emit(MCInst &Inst, SMLoc const &Loc, MCStreamer &Out) const {
  Inst.setLoc(Loc);
...
bool BLUCPUAsmParser::MatchAndEmitInstruction(SMLoc Loc, unsigned &Opcode,
  ...
  case Match_InvalidRegisterOnTiny:
    return Error(Loc, "invalid register on blucputiny");
  default:
    return true;
  }
}
...
int BLUCPUAsmParser::parseRegisterName(MCRegister (*matchFn)(StringRef)) {
  StringRef Name = Parser.getTok().getString();

  int RegNum = matchFn(Name);

  // GCC supports case insensitive register names. Some of the BLUCPU registers
...
int BLUCPUAsmParser::parseRegisterName() {
...
int BLUCPUAsmParser::parseRegister(bool RestoreOnFailure) {
  int RegNum = BLUCPU::NoRegister;
  ...
  if (Parser.getTok().is(AsmToken::Identifier)) {
    ...
    if (Parser.getLexer().peekTok().is(AsmToken::Colon)) {
      ...
      if (RegNum == BLUCPU::NoRegister && RestoreOnFailure) {
...
bool BLUCPUAsmParser::tryParseRegisterOperand(OperandVector &Operands) {
...
bool BLUCPUAsmParser::tryParseExpression(OperandVector &Operands, int64_t offset) {
  ...
  Operands.push_back(BLUCPUOperand::CreateImm(Expression, S, E));
  return false;
}

bool BLUCPUAsmParser::tryParseRelocExpression(OperandVector &Operands) {
  ...
  ModifierKind = BLUCPUMCExpr::getKindByName(ModifierName);

  if (ModifierKind != BLUCPUMCExpr::VK_BLUCPU_None) {
    Parser.Lex();
    ...
    if (Parser.getTok().getString() == GENERATE_STUBS &&
      ...
      ModifierKind = BLUCPUMCExpr::getKindByName(GSModName);
      if (ModifierKind != BLUCPUMCExpr::VK_BLUCPU_None)
  ...
  MCExpr const *Expression =
      BLUCPUMCExpr::create(ModifierKind, InnerExpression, isNegated, getContext());
...
bool BLUCPUAsmParser::parseOperand(OperandVector &Operands, bool maybeReg) {
  ...
  case AsmToken::Minus: {
    ...
    Operands.push_back(BLUCPUOperand::CreateToken(Parser.getTok().getString(),
...
ParseStatus BLUCPUAsmParser::parseMemriOperand(OperandVector &Operands) {
  ...
  Operands.push_back(BLUCPUOperand::CreateMemri(RegNo, Expression, S, E));

  return ParseStatus::Success;
}

bool BLUCPUAsmParser::parseRegister(MCRegister &Reg, SMLoc &StartLoc,
                                 SMLoc &EndLoc) {
  StartLoc = Parser.getTok().getLoc();
  Reg = parseRegister(/*RestoreOnFailure=*/false);
  EndLoc = Parser.getTok().getLoc();

  return Reg == BLUCPU::NoRegister;
}

ParseStatus BLUCPUAsmParser::tryParseRegister(MCRegister &Reg, SMLoc &StartLoc,
                                           SMLoc &EndLoc) {
  StartLoc = Parser.getTok().getLoc();
  Reg = parseRegister(/*RestoreOnFailure=*/true);
  EndLoc = Parser.getTok().getLoc();

  if (Reg == BLUCPU::NoRegister)
    return ParseStatus::NoMatch;
  return ParseStatus::Success;
}

void BLUCPUAsmParser::eatComma() {
...
bool BLUCPUAsmParser::ParseInstruction(ParseInstructionInfo &Info,
                                    StringRef Mnemonic, SMLoc NameLoc,
                                    OperandVector &Operands) {
  Operands.push_back(BLUCPUOperand::CreateToken(Mnemonic, NameLoc));
...
ParseStatus BLUCPUAsmParser::parseDirective(llvm::AsmToken DirectiveID) {
...
ParseStatus BLUCPUAsmParser::parseLiteralValues(unsigned SizeInBytes, SMLoc L) {
  MCAsmParser &Parser = getParser();
  BLUCPUMCELFStreamer &BLUCPUStreamer =
      static_cast<BLUCPUMCELFStreamer &>(Parser.getStreamer());
  AsmToken Tokens[2];
  ...
  if (ReadCount == 2 && Parser.getTok().getKind() == AsmToken::Identifier &&
    ...
    BLUCPUStreamer.emitValueForModiferKind(Symbol, SizeInBytes, L,
                                        BLUCPUMCExpr::VK_BLUCPU_None);
    return ParseStatus::NoMatch;
  }
  ...
  if (Parser.getTok().getKind() == AsmToken::Identifier &&
      Parser.getLexer().peekTok().getKind() == AsmToken::LParen) {
    StringRef ModifierName = Parser.getTok().getString();
    BLUCPUMCExpr::VariantKind ModifierKind =
        BLUCPUMCExpr::getKindByName(ModifierName);
    if (ModifierKind != BLUCPUMCExpr::VK_BLUCPU_None) {
      Parser.Lex();
    ...
    BLUCPUStreamer.emitValueForModiferKind(Symbol, SizeInBytes, L, ModifierKind);
...
extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeBLUCPUAsmParser() {
  RegisterMCAsmParser<BLUCPUAsmParser> X(getTheBLUCPUTarget());
}
...
// Uses enums defined in BLUCPUGenAsmMatcher.inc
unsigned BLUCPUAsmParser::validateTargetOperandClass(MCParsedAsmOperand &AsmOp,
                                                  unsigned ExpectedKind) {
  BLUCPUOperand &Op = static_cast<BLUCPUOperand &>(AsmOp);
  ...
  if (Op.isReg()) {
    ...
    if (isSubclass(Expected, MCK_DREGS)) {
      ...
      if (correspondingDREG != BLUCPU::NoRegister) {
