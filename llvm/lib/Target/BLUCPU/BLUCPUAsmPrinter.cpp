//===-- BLUCPUAsmPrinter.cpp - BLUCPU LLVM assembly writer ----------------------===//
//
...
// of machine-dependent LLVM code to GAS-format BLUCPU assembly language.
//
//===----------------------------------------------------------------------===//
...
void BLUCPUAsmPrinter::printOperand(const MachineInstr *MI, unsigned OpNo,
                                 raw_ostream &O) {
  const MachineOperand &MO = MI->getOperand(OpNo);

  switch (MO.getType()) {
  case MachineOperand::MO_Register:
    O << BLUCPUInstPrinter::getPrettyRegisterName(MO.getReg(), MRI);
    break;
...
bool BLUCPUAsmPrinter::PrintAsmOperand(const MachineInstr *MI, unsigned OpNum,
...
bool BLUCPUAsmPrinter::PrintAsmMemoryOperand(const MachineInstr *MI,
...
void BLUCPUAsmPrinter::emitInstruction(const MachineInstr *MI) {
  BLUCPU_MC::verifyInstructionPredicates(MI->getOpcode(),
                                      getSubtargetInfo().getFeatureBits());

  BLUCPUMCInstLower MCInstLowering(OutContext, *this);
...
const MCExpr *BLUCPUAsmPrinter::lowerConstant(const Constant *CV) {
  MCContext &Ctx = OutContext;

  if (const GlobalValue *GV = dyn_cast<GlobalValue>(CV)) {
    bool IsProgMem = GV->getAddressSpace() == BLUCPU::ProgramMemory;
    if (IsProgMem) {
      const MCExpr *Expr = MCSymbolRefExpr::create(getSymbol(GV), Ctx);
      return BLUCPUMCExpr::create(BLUCPUMCExpr::VK_BLUCPU_PM, Expr, false, Ctx);
    }
  }
...
void BLUCPUAsmPrinter::emitXXStructor(const DataLayout &DL, const Constant *CV) {
