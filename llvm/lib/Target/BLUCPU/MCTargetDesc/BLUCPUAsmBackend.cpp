//===-- BLUCPUAsmBackend.cpp - BLUCPU Asm Backend  ------------------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the BLUCPUAsmBackend class.
//
//===----------------------------------------------------------------------===//

#include "MCTargetDesc/BLUCPUAsmBackend.h"
...
static void adjustBranch(unsigned Size, const MCFixup &Fixup, uint64_t &Value,
  ...
  BLUCPU::fixups::adjustBranchTarget(Value);
}
...
/// 7-bit data space address fixup for the LDS/STS instructions on BLUCPUTiny.
///
/// Resolves to:
/// 1010 ikkk dddd kkkk
...
void BLUCPUAsmBackend::adjustFixupValue(const MCFixup &Fixup,
                                     const MCValue &Target, uint64_t &Value,
                                     MCContext *Ctx) const {
  // The size of the fixup in bits.
  uint64_t Size = BLUCPUAsmBackend::getFixupKindInfo(Fixup.getKind()).TargetSize;

  unsigned Kind = Fixup.getKind();
  switch (Kind) {
  default:
    llvm_unreachable("unhandled fixup");
  case BLUCPU::fixup_7_pcrel:
    adjust::fixup_7_pcrel(Size, Fixup, Value, Ctx);
    break;
  case BLUCPU::fixup_13_pcrel:
    adjust::fixup_13_pcrel(Size, Fixup, Value, Ctx);
    break;
  case BLUCPU::fixup_call:
    adjust::fixup_call(Size, Fixup, Value, Ctx);
    break;
  case BLUCPU::fixup_ldi:
    adjust::ldi::fixup(Size, Fixup, Value, Ctx);
    break;
  case BLUCPU::fixup_lo8_ldi:
    adjust::ldi::lo8(Size, Fixup, Value, Ctx);
    break;
  case BLUCPU::fixup_lo8_ldi_pm:
  case BLUCPU::fixup_lo8_ldi_gs:
    adjust::pm(Value);
    adjust::ldi::lo8(Size, Fixup, Value, Ctx);
    break;
  case BLUCPU::fixup_hi8_ldi:
    adjust::ldi::hi8(Size, Fixup, Value, Ctx);
    break;
  case BLUCPU::fixup_hi8_ldi_pm:
  case BLUCPU::fixup_hi8_ldi_gs:
    adjust::pm(Value);
    adjust::ldi::hi8(Size, Fixup, Value, Ctx);
    break;
  case BLUCPU::fixup_hh8_ldi:
  case BLUCPU::fixup_hh8_ldi_pm:
    if (Kind == BLUCPU::fixup_hh8_ldi_pm)
      adjust::pm(Value);

    adjust::ldi::hh8(Size, Fixup, Value, Ctx);
    break;
  case BLUCPU::fixup_ms8_ldi:
    adjust::ldi::ms8(Size, Fixup, Value, Ctx);
    break;

  case BLUCPU::fixup_lo8_ldi_neg:
  case BLUCPU::fixup_lo8_ldi_pm_neg:
    if (Kind == BLUCPU::fixup_lo8_ldi_pm_neg)
      adjust::pm(Value);

    adjust::ldi::neg(Value);
    adjust::ldi::lo8(Size, Fixup, Value, Ctx);
    break;
  case BLUCPU::fixup_hi8_ldi_neg:
  case BLUCPU::fixup_hi8_ldi_pm_neg:
    if (Kind == BLUCPU::fixup_hi8_ldi_pm_neg)
      adjust::pm(Value);

    adjust::ldi::neg(Value);
    adjust::ldi::hi8(Size, Fixup, Value, Ctx);
    break;
  case BLUCPU::fixup_hh8_ldi_neg:
  case BLUCPU::fixup_hh8_ldi_pm_neg:
    if (Kind == BLUCPU::fixup_hh8_ldi_pm_neg)
      adjust::pm(Value);

    adjust::ldi::neg(Value);
    adjust::ldi::hh8(Size, Fixup, Value, Ctx);
    break;
  case BLUCPU::fixup_ms8_ldi_neg:
    adjust::ldi::neg(Value);
    adjust::ldi::ms8(Size, Fixup, Value, Ctx);
    break;
  case BLUCPU::fixup_16:
    adjust::unsigned_width(16, Value, std::string("port number"), Fixup, Ctx);

    Value &= 0xffff;
    break;
  case BLUCPU::fixup_16_pm:
...
BLUCPUAsmBackend::createObjectTargetWriter() const {
  return createBLUCPUELFObjectWriter(MCELFObjectTargetWriter::getOSABI(OSType));
}

void BLUCPUAsmBackend::applyFixup(const MCAssembler &Asm, const MCFixup &Fixup,
...
std::optional<MCFixupKind> BLUCPUAsmBackend::getFixupKind(StringRef Name) const {
  return std::nullopt;
}

MCFixupKindInfo const &BLUCPUAsmBackend::getFixupKindInfo(MCFixupKind Kind) const {
  // NOTE: Many BLUCPU fixups work on sets of non-contignous bits. We work around
  // this by saying that the fixup is the size of the entire instruction.
  const static MCFixupKindInfo Infos[BLUCPU::NumTargetFixupKinds] = {
  ...
  // Fixup kinds from .reloc directive are like R_BLUCPU_NONE. They do not require
...
bool BLUCPUAsmBackend::writeNopData(raw_ostream &OS, uint64_t Count,
                                 const MCSubtargetInfo *STI) const {
...
bool BLUCPUAsmBackend::shouldForceRelocation(const MCAssembler &Asm,
                                          const MCFixup &Fixup,
                                          const MCValue &Target,
...
MCAsmBackend *createBLUCPUAsmBackend(const Target &T, const MCSubtargetInfo &STI,
                                  const MCRegisterInfo &MRI,
                                  const llvm::MCTargetOptions &TO) {
  return new BLUCPUAsmBackend(STI.getTargetTriple().getOS());
}
