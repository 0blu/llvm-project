//===-- BLUCPUMCCodeEmitter.cpp - Convert BLUCPU Code to Machine Code -----------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the BLUCPUMCCodeEmitter class.
//
//===----------------------------------------------------------------------===//
...
unsigned
BLUCPUMCCodeEmitter::loadStorePostEncoder(const MCInst &MI, unsigned EncodedValue,
  ...
  bool IsRegX = MI.getOperand(0).getReg() == BLUCPU::R27R26 ||
                MI.getOperand(1).getReg() == BLUCPU::R27R26;

  bool IsPredec = Opcode == BLUCPU::LDRdPtrPd || Opcode == BLUCPU::STPtrPdRr;
  bool IsPostinc = Opcode == BLUCPU::LDRdPtrPi || Opcode == BLUCPU::STPtrPiRr;
...
unsigned
BLUCPUMCCodeEmitter::encodeRelCondBrTarget(const MCInst &MI, unsigned OpNo,
  ...
  BLUCPU::fixups::adjustBranchTarget(target);
  return target;
}

unsigned BLUCPUMCCodeEmitter::encodeLDSTPtrReg(const MCInst &MI, unsigned OpNo,
                                            SmallVectorImpl<MCFixup> &Fixups,
                                            const MCSubtargetInfo &STI) const {
...
unsigned BLUCPUMCCodeEmitter::encodeMemri(const MCInst &MI, unsigned OpNo,
                                       SmallVectorImpl<MCFixup> &Fixups,
                                       const MCSubtargetInfo &STI) const {
  ...
  if (OffsetOp.isImm()) {
    ...
    Fixups.push_back(MCFixup::create(0, OffsetOp.getExpr(),
                                     MCFixupKind(BLUCPU::fixup_6), MI.getLoc()));
  } else {
...
unsigned BLUCPUMCCodeEmitter::encodeComplement(const MCInst &MI, unsigned OpNo,
                                            SmallVectorImpl<MCFixup> &Fixups,
                                            const MCSubtargetInfo &STI) const {
...
unsigned BLUCPUMCCodeEmitter::encodeImm(const MCInst &MI, unsigned OpNo,
                                     SmallVectorImpl<MCFixup> &Fixups,
                                     const MCSubtargetInfo &STI) const {
  auto MO = MI.getOperand(OpNo);

  if (MO.isExpr()) {
    if (isa<BLUCPUMCExpr>(MO.getExpr())) {
      // If the expression is already an BLUCPUMCExpr (i.e. a lo8(symbol),
...
unsigned BLUCPUMCCodeEmitter::encodeCallTarget(const MCInst &MI, unsigned OpNo,
                                            SmallVectorImpl<MCFixup> &Fixups,
                                            const MCSubtargetInfo &STI) const {
  auto MO = MI.getOperand(OpNo);

  if (MO.isExpr()) {
    MCFixupKind FixupKind = static_cast<MCFixupKind>(BLUCPU::fixup_call);
...
unsigned BLUCPUMCCodeEmitter::getExprOpValue(const MCExpr *Expr,
                                          SmallVectorImpl<MCFixup> &Fixups,
                                          const MCSubtargetInfo &STI) const {
  ...
  if (Kind == MCExpr::Target) {
    BLUCPUMCExpr const *BLUCPUExpr = cast<BLUCPUMCExpr>(Expr);
    int64_t Result;
    if (BLUCPUExpr->evaluateAsConstant(Result)) {
      return Result;
    }

    MCFixupKind FixupKind = static_cast<MCFixupKind>(BLUCPUExpr->getFixupKind());
    Fixups.push_back(MCFixup::create(0, BLUCPUExpr, FixupKind));
    return 0;
  }
...
unsigned BLUCPUMCCodeEmitter::getMachineOpValue(const MCInst &MI,
                                             const MCOperand &MO,
                                             SmallVectorImpl<MCFixup> &Fixups,
                                             const MCSubtargetInfo &STI) const {
  if (MO.isReg())
...
void BLUCPUMCCodeEmitter::encodeInstruction(const MCInst &MI,
...
MCCodeEmitter *createBLUCPUMCCodeEmitter(const MCInstrInfo &MCII,
                                      MCContext &Ctx) {
  return new BLUCPUMCCodeEmitter(MCII, Ctx);
}

#include "BLUCPUGenMCCodeEmitter.inc"
