//===-- BLUCPUInstPrinter.cpp - Convert BLUCPU MCInst to assembly syntax --------===//
//
...
void BLUCPUInstPrinter::printPCRelImm(const MCInst *MI, unsigned OpNo,
                                   raw_ostream &O) {
  if (OpNo >= MI->size()) {
    // Not all operands are correctly disassembled at the moment. This means
    // that some machine instructions won't have all the necessary operands
...
void BLUCPUInstPrinter::printMemri(const MCInst *MI, unsigned OpNo,
                                raw_ostream &O) {
