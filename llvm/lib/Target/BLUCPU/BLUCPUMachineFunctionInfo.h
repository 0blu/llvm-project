//===-- BLUCPUMachineFuctionInfo.h - BLUCPU machine function info -----*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This file declares BLUCPU-specific per-machine-function information.
//
//===----------------------------------------------------------------------===//
...
public:
  BLUCPUMachineFunctionInfo(const Function &F, const TargetSubtargetInfo *STI)
    ...
    this->IsInterruptHandler = false; //CallConv == CallingConv::BLUCPU_INTR || F.hasFnAttribute("interrupt");
    this->IsSignalHandler = false; //CallConv == CallingConv::BLUCPU_SIGNAL || F.hasFnAttribute("signal");
  }
