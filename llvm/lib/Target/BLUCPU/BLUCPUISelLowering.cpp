...
SDValue BLUCPUTargetLowering::LowerShifts(SDValue Op, SelectionDAG &DAG) const {
  unsigned Opc8;
  ...
  if (!isa<ConstantSDNode>(N->getOperand(1))) {
    ...
    case ISD::SHL:
      return DAG.getNode(BLUCPUISD::LSLLOOP, dl, VT, N->getOperand(0),
                         N->getOperand(1));
    case ISD::SRL:
      return DAG.getNode(BLUCPUISD::LSRLOOP, dl, VT, N->getOperand(0),
                         N->getOperand(1));
    case ISD::ROTL: {
      ...
      return DAG.getNode(BLUCPUISD::ROLLOOP, dl, VT, N->getOperand(0), Amt);
    }
    case ISD::ROTR: {
      SDValue Amt = N->getOperand(1);
      EVT AmtVT = Amt.getValueType();
      Amt = DAG.getNode(ISD::AND, dl, AmtVT, Amt,
                        DAG.getConstant(VT.getSizeInBits() - 1, dl, AmtVT));
      return DAG.getNode(BLUCPUISD::RORLOOP, dl, VT, N->getOperand(0), Amt);
    }
    case ISD::SRA:
      return DAG.getNode(BLUCPUISD::ASRLOOP, dl, VT, N->getOperand(0),
                         N->getOperand(1));
    }
  }
  ...
  if (VT.getSizeInBits() == 8) {
    if (Op.getOpcode() == ISD::SHL && 4 <= ShiftAmount && ShiftAmount < 7) {
      ...
      Victim = DAG.getNode(BLUCPUISD::SWAP, dl, VT, Victim);
      Victim =
      ...
      Victim = DAG.getNode(BLUCPUISD::SWAP, dl, VT, Victim);
      Victim =
          DAG.getNode(BLUCPUISD::ROR, dl, VT, Victim, DAG.getConstant(1, dl, VT));
      ShiftAmount = 0;
    } else if (Op.getOpcode() == ISD::ROTR && ShiftAmount == 3) {
      // Optimize right rotation 3 bits to swap then left rotation 1 bit.
      Victim = DAG.getNode(BLUCPUISD::SWAP, dl, VT, Victim);
      Victim =
          DAG.getNode(BLUCPUISD::ROL, dl, VT, Victim, DAG.getConstant(1, dl, VT));
      ShiftAmount = 0;
    ...
    if (Op.getOpcode() == ISD::SRA)
      ...
      switch (ShiftAmount) {
      case 15:
        Victim = DAG.getNode(BLUCPUISD::ASRWN, dl, VT, Victim,
    ...
    if (4 <= ShiftAmount && ShiftAmount < 8)
      switch (Op.getOpcode()) {
      case ISD::SHL:
        Victim = DAG.getNode(BLUCPUISD::LSLWN, dl, VT, Victim,
                             DAG.getConstant(4, dl, VT));
        ShiftAmount -= 4;
        break;
      case ISD::SRL:
        Victim = DAG.getNode(BLUCPUISD::LSRWN, dl, VT, Victim,
                             DAG.getConstant(4, dl, VT));
        ShiftAmount -= 4;
        break;
      default:
...
SDValue BLUCPUTargetLowering::LowerDivRem(SDValue Op, SelectionDAG &DAG) const {
...
SDValue BLUCPUTargetLowering::LowerGlobalAddress(SDValue Op,
  ...
  return DAG.getNode(BLUCPUISD::WRAPPER, SDLoc(Op), getPointerTy(DL), Result);
}

SDValue BLUCPUTargetLowering::LowerBlockAddress(SDValue Op,
                                             SelectionDAG &DAG) const {
  auto DL = DAG.getDataLayout();
...
/// IntCCToBLUCPUCC - Convert a DAG integer condition code to an BLUCPU CC.
static BLUCPUCC::CondCodes intCCToBLUCPUCC(ISD::CondCode CC) {
  switch (CC) {
  default:
...
SDValue BLUCPUTargetLowering::getBLUCPUCmp(SDValue LHS, SDValue RHS,
  ...
  if (LHS.getSimpleValueType() == MVT::i16 && isa<ConstantSDNode>(RHS)) {
    ...
    SDValue RHShi = (Imm & 0xff00) == 0
                        ...
                        : DAG.getNode(ISD::EXTRACT_ELEMENT, DL, MVT::i8, RHS,
                                      DAG.getIntPtrConstant(1, DL));
    Cmp = DAG.getNode(BLUCPUISD::CMP, DL, MVT::Glue, LHSlo, RHSlo);
    Cmp = DAG.getNode(BLUCPUISD::CMPC, DL, MVT::Glue, LHShi, RHShi, Cmp);
    ...
    Cmp = DAG.getNode(BLUCPUISD::CMP, DL, MVT::Glue, LHS, RHS);
  }

  return Cmp;
}

/// Returns appropriate BLUCPU CMP/CMPC nodes and corresponding condition code for
/// the given operands.
SDValue BLUCPUTargetLowering::getBLUCPUCmp(SDValue LHS, SDValue RHS, ISD::CondCode CC,
                                     SDValue &BLUCPUcc, SelectionDAG &DAG,
                                     SDLoc DL) const {
  SDValue Cmp;
  ...
  case ISD::SETGT: {
    if (const ConstantSDNode *C = dyn_cast<ConstantSDNode>(RHS)) {
      ...
      case -1: {
        ...
        UseTest = true;
        BLUCPUcc = DAG.getConstant(BLUCPUCC::COND_PL, DL, MVT::i8);
        break;
      }
      case 0: {
  ...
  case ISD::SETLT: {
    if (const ConstantSDNode *C = dyn_cast<ConstantSDNode>(RHS)) {
      ...
      case 0: {
        ...
        UseTest = true;
        BLUCPUcc = DAG.getConstant(BLUCPUCC::COND_MI, DL, MVT::i8);
        break;
      }
      }
    }
  ...
  if (VT == MVT::i32) {
    ...
    if (UseTest) {
      ...
      SDValue Top = DAG.getNode(ISD::EXTRACT_ELEMENT, DL, MVT::i8, LHShi,
                                DAG.getIntPtrConstant(1, DL));
      Cmp = DAG.getNode(BLUCPUISD::TST, DL, MVT::Glue, Top);
    } else {
      Cmp = getBLUCPUCmp(LHSlo, RHSlo, DAG, DL);
      Cmp = DAG.getNode(BLUCPUISD::CMPC, DL, MVT::Glue, LHShi, RHShi, Cmp);
    }
  } else if (VT == MVT::i64) {
    ...
    if (UseTest) {
      ...
      Cmp = getBLUCPUCmp(LHS0, RHS0, DAG, DL);
      Cmp = DAG.getNode(BLUCPUISD::CMPC, DL, MVT::Glue, LHS1, RHS1, Cmp);
      Cmp = DAG.getNode(BLUCPUISD::CMPC, DL, MVT::Glue, LHS2, RHS2, Cmp);
      Cmp = DAG.getNode(BLUCPUISD::CMPC, DL, MVT::Glue, LHS3, RHS3, Cmp);
    }
  } else if (VT == MVT::i8 || VT == MVT::i16) {
    if (UseTest) {
      // When using tst we only care about the highest part.
      Cmp = DAG.getNode(BLUCPUISD::TST, DL, MVT::Glue,
                        (VT == MVT::i8)
                            ? LHS
                            : DAG.getNode(ISD::EXTRACT_ELEMENT, DL, MVT::i8,
                                          LHS, DAG.getIntPtrConstant(1, DL)));
    } else {
      Cmp = getBLUCPUCmp(LHS, RHS, DAG, DL);
    }
  } else {
    llvm_unreachable("Invalid comparison size");
  }

  // When using a test instruction BLUCPUcc is already set.
  if (!UseTest) {
    BLUCPUcc = DAG.getConstant(intCCToBLUCPUCC(CC), DL, MVT::i8);
  }

  return Cmp;
}

SDValue BLUCPUTargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue TargetCC;
  SDValue Cmp = getBLUCPUCmp(LHS, RHS, CC, TargetCC, DAG, dl);

  return DAG.getNode(BLUCPUISD::BRCOND, dl, MVT::Other, Chain, Dest, TargetCC,
                     Cmp);
}

SDValue BLUCPUTargetLowering::LowerSELECT_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(BLUCPUISD::SELECT_CC, dl, VTs, Ops);
}

SDValue BLUCPUTargetLowering::LowerSETCC(SDValue Op, SelectionDAG &DAG) const {
  SDValue LHS = Op.getOperand(0);
  SDValue RHS = Op.getOperand(1);
...
SDValue BLUCPUTargetLowering::LowerVASTART(SDValue Op, SelectionDAG &DAG) const {
...
SDValue BLUCPUTargetLowering::LowerINLINEASM(SDValue Op, SelectionDAG &DAG) const {
...
static void analyzeArguments(TargetLowering::CallLoweringInfo *CLI,
  ...
  if (Tiny) {
    ...
    RegList8 = ArrayRef(RegList8BLUCPU);
    RegList16 = ArrayRef(RegList16BLUCPU);
  }
...
static void analyzeReturnValues(const SmallVectorImpl<ArgT> &Args,
  ...
  if (Tiny)
    assert(TotalBytes <= 4 &&
           "return values greater than 4 bytes cannot be lowered on BLUCPUTiny");
  else
    assert(TotalBytes <= 8 &&
           "return values greater than 8 bytes cannot be lowered on BLUCPU");
...
bool BLUCPUTargetLowering::CanLowerReturn(
  ...
  //  return CCInfo.CheckReturn(Outs, RetCC_BLUCPU_BUILTIN);
  //}
...
MachineBasicBlock *BLUCPUTargetLowering::insertShift(MachineInstr &MI, MachineBasicBlock *BB) const {
  ...
  default:
    llvm_unreachable("Invalid shift opcode!");
  case BLUCPU::Lsl8:
    Opc = BLUCPU::ADDRdRr; // LSL is an alias of ADD Rd, Rd
    RC = &BLUCPU::GPR8RegClass;
  ...
  Register ShiftAmtReg = RI.createVirtualRegister(&BLUCPU::GPR8RegClass);
  Register ShiftAmtReg2 = RI.createVirtualRegister(&BLUCPU::GPR8RegClass);
  ...
  // BB:
  // rjmp CheckBB
  BuildMI(BB, dl, TII.get(BLUCPU::RJMPk)).addMBB(CheckBB);
  ...
  BuildMI(CheckBB, dl, TII.get(BLUCPU::PHI), ShiftReg)
      .addReg(SrcReg)
      .addMBB(BB)
      .addReg(ShiftReg2)
      .addMBB(LoopBB);
  BuildMI(CheckBB, dl, TII.get(BLUCPU::PHI), ShiftAmtReg)
      .addReg(ShiftAmtSrcReg)
      .addMBB(BB)
      .addReg(ShiftAmtReg2)
      .addMBB(LoopBB);
  BuildMI(CheckBB, dl, TII.get(BLUCPU::PHI), DstReg)
      .addReg(SrcReg)
      .addMBB(BB)
      .addReg(ShiftReg2)
      .addMBB(LoopBB);

  BuildMI(CheckBB, dl, TII.get(BLUCPU::DECRd), ShiftAmtReg2).addReg(ShiftAmtReg);
  BuildMI(CheckBB, dl, TII.get(BLUCPU::BRPLk)).addMBB(LoopBB);

  MI.eraseFromParent(); // The pseudo instruction is gone now.
  return RemBB;
}

// Do a multibyte BLUCPU shift. Insert shift instructions and put the output
// registers in the Regs array.
// Because BLUCPU does not have a normal shift instruction (only a single bit shift
...
// https://aykevl.nl/2021/02/blucpu-bitshift
static void insertMultibyteShift(MachineInstr &MI, MachineBasicBlock *BB,
  ...
  const BLUCPUSubtarget &STI = BB->getParent()->getSubtarget<BLUCPUSubtarget>();
  ...
  Register ZeroReg = MRI.createVirtualRegister(&BLUCPU::GPR8RegClass);
  BuildMI(*BB, MI, dl, TII.get(BLUCPU::COPY), ZeroReg)
  ...
  if (ShiftLeft && (ShiftAmt % 8) >= 6) {
    ...
    Register LowByte = MRI.createVirtualRegister(&BLUCPU::GPR8RegClass);
    BuildMI(*BB, MI, dl, TII.get(BLUCPU::RORRd), LowByte).addReg(ZeroReg);

    // Shift one more to the right if this is a modulo-6 shift.
    if (ShiftAmt % 8 == 6) {
      insertMultibyteShift(MI, BB, ShiftRegs, ISD::SRL, 1);
      Register NewLowByte = MRI.createVirtualRegister(&BLUCPU::GPR8RegClass);
      BuildMI(*BB, MI, dl, TII.get(BLUCPU::RORRd), NewLowByte).addReg(LowByte);
      LowByte = NewLowByte;
    }
  ...
  if (!ShiftLeft && (ShiftAmt % 8) >= 6) {
    ...
    // are either 0 or 1).
    Register HighByte = MRI.createVirtualRegister(&BLUCPU::GPR8RegClass);
    Register ExtByte = 0;
    if (ArithmeticShift) {
      // Sign-extend bit that was shifted out last.
      BuildMI(*BB, MI, dl, TII.get(BLUCPU::SBCRdRr), HighByte)
      ...
      BuildMI(*BB, MI, dl, TII.get(BLUCPU::ADCRdRr), HighByte)
          .addReg(ExtByte)
          .addReg(ExtByte);
    }
    ...
    if (ShiftAmt % 8 == 6) {
      ...
      Register NewExt = MRI.createVirtualRegister(&BLUCPU::GPR8RegClass);
      BuildMI(*BB, MI, dl, TII.get(BLUCPU::ADCRdRr), NewExt)
          .addReg(HighByte)
          .addReg(HighByte);
      HighByte = NewExt;
    }
  ...
  if (!ShiftLeft && ShiftAmt >= 8) {
    if (ArithmeticShift) {
      ...
      ShrExtendReg = MRI.createVirtualRegister(&BLUCPU::GPR8RegClass);
      Register Tmp = MRI.createVirtualRegister(&BLUCPU::GPR8RegClass);
      BuildMI(*BB, MI, dl, TII.get(BLUCPU::ADDRdRr), Tmp)
          .addReg(Regs[0].first, 0, Regs[0].second)
          .addReg(Regs[0].first, 0, Regs[0].second);
      BuildMI(*BB, MI, dl, TII.get(BLUCPU::SBCRdRr), ShrExtendReg)
          .addReg(Tmp)
          .addReg(Tmp);
    } else {
  ...
  if (!ArithmeticShift && ShiftAmt >= 4) {
    ...
    for (size_t I = 0; I < Regs.size(); I++) {
      ...
      Register SwapReg = MRI.createVirtualRegister(&BLUCPU::LD8RegClass);
      BuildMI(*BB, MI, dl, TII.get(BLUCPU::SWAPRd), SwapReg)
          .addReg(Regs[Idx].first, 0, Regs[Idx].second);
      if (I != 0) {
        Register R = MRI.createVirtualRegister(&BLUCPU::GPR8RegClass);
        BuildMI(*BB, MI, dl, TII.get(BLUCPU::EORRdRr), R)
            .addReg(Prev)
            .addReg(SwapReg);
        Prev = R;
      }
      Register AndReg = MRI.createVirtualRegister(&BLUCPU::LD8RegClass);
      BuildMI(*BB, MI, dl, TII.get(BLUCPU::ANDIRdK), AndReg)
          .addReg(SwapReg)
  ...
  while (ShiftLeft && ShiftAmt) {
    ...
    for (ssize_t I = Regs.size() - 1; I >= 0; I--) {
      Register Out = MRI.createVirtualRegister(&BLUCPU::GPR8RegClass);
      Register In = Regs[I].first;
      Register InSubreg = Regs[I].second;
      if (I == (ssize_t)Regs.size() - 1) { // first iteration
        BuildMI(*BB, MI, dl, TII.get(BLUCPU::ADDRdRr), Out)
            .addReg(In, 0, InSubreg)
            .addReg(In, 0, InSubreg);
      } else {
        BuildMI(*BB, MI, dl, TII.get(BLUCPU::ADCRdRr), Out)
            .addReg(In, 0, InSubreg)
            .addReg(In, 0, InSubreg);
      }
  ...
  while (!ShiftLeft && ShiftAmt) {
    ...
    for (size_t I = 0; I < Regs.size(); I++) {
      ...
      Register InSubreg = Regs[I].second;
      if (I == 0) {
        unsigned Opc = ArithmeticShift ? BLUCPU::ASRRd : BLUCPU::LSRRd;
        BuildMI(*BB, MI, dl, TII.get(Opc), Out).addReg(In, 0, InSubreg);
      } else {
        BuildMI(*BB, MI, dl, TII.get(BLUCPU::RORRd), Out).addReg(In, 0, InSubreg);
      }
      Regs[I] = std::pair(Out, 0);
    }
    ShiftAmt--;
...
MachineBasicBlock *
BLUCPUTargetLowering::insertWideShift(MachineInstr &MI,
  ...
  std::array<std::pair<Register, int>, 4> Registers = {
      std::pair(MI.getOperand(3).getReg(), BLUCPU::sub_hi),
      std::pair(MI.getOperand(3).getReg(), BLUCPU::sub_lo),
      std::pair(MI.getOperand(2).getReg(), BLUCPU::sub_hi),
      std::pair(MI.getOperand(2).getReg(), BLUCPU::sub_lo),
  };
  ...
  // already equal to or faster than blucpu-gcc in all cases except ashr 8).
  if (Opc != ISD::SHL &&
      (Opc != ISD::SRA || (ShiftAmt < 16 || ShiftAmt >= 22))) {
    // Use the resulting registers starting with the least significant byte.
    BuildMI(*BB, MI, dl, TII.get(BLUCPU::REG_SEQUENCE), MI.getOperand(0).getReg())
        .addReg(Registers[3].first, 0, Registers[3].second)
        .addImm(BLUCPU::sub_lo)
        .addReg(Registers[2].first, 0, Registers[2].second)
        .addImm(BLUCPU::sub_hi);
    BuildMI(*BB, MI, dl, TII.get(BLUCPU::REG_SEQUENCE), MI.getOperand(1).getReg())
...
MachineBasicBlock *BLUCPUTargetLowering::insertMul(MachineInstr &MI,
                                                MachineBasicBlock *BB) const {
  const TargetInstrInfo &TII = *Subtarget.getInstrInfo();
  ...
  if (isCopyMulResult(I))
    ++I;
  BuildMI(*BB, I, MI.getDebugLoc(), TII.get(BLUCPU::EORRdRr), BLUCPU::R1)
      .addReg(BLUCPU::R1)
      .addReg(BLUCPU::R1);
  return BB;
}
...
MachineBasicBlock *
BLUCPUTargetLowering::insertCopyZero(MachineInstr &MI,
                                  MachineBasicBlock *BB) const {
  const TargetInstrInfo &TII = *Subtarget.getInstrInfo();
  MachineBasicBlock::iterator I(MI);
  BuildMI(*BB, I, MI.getDebugLoc(), TII.get(BLUCPU::COPY))
      .add(MI.getOperand(0))
...
// interrupts. This works because all BLUCPU microcontrollers are single core.
MachineBasicBlock *BLUCPUTargetLowering::insertAtomicArithmeticOp(
  ...
  const TargetRegisterClass *RC =
      (Width == 8) ? &BLUCPU::GPR8RegClass : &BLUCPU::DREGSRegClass;
  unsigned LoadOpcode = (Width == 8) ? BLUCPU::LDRdPtr : BLUCPU::LDWRdPtr;
  unsigned StoreOpcode = (Width == 8) ? BLUCPU::STPtrRr : BLUCPU::STWPtrRr;

  // Disable interrupts.
  BuildMI(*BB, I, dl, TII.get(BLUCPU::INRdA), Subtarget.getTmpRegister())
      .addImm(Subtarget.getIORegSREG());
  BuildMI(*BB, I, dl, TII.get(BLUCPU::BCLRs)).addImm(7);
  ...
  // Restore interrupts.
  BuildMI(*BB, I, dl, TII.get(BLUCPU::OUTARr))
      .addImm(Subtarget.getIORegSREG())
...
BLUCPUTargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
  ...
  case BLUCPU::MULRdRr:
  case BLUCPU::MULSRdRr:
    return insertMul(MI, MBB);
  case BLUCPU::CopyZero:
    return insertCopyZero(MI, MBB);
  case BLUCPU::AtomicLoadAdd8:
    return insertAtomicArithmeticOp(MI, MBB, BLUCPU::ADDRdRr, 8);
  case BLUCPU::AtomicLoadAdd16:
    return insertAtomicArithmeticOp(MI, MBB, BLUCPU::ADDWRdRr, 16);
  case BLUCPU::AtomicLoadSub8:
    return insertAtomicArithmeticOp(MI, MBB, BLUCPU::SUBRdRr, 8);
  case BLUCPU::AtomicLoadSub16:
    return insertAtomicArithmeticOp(MI, MBB, BLUCPU::SUBWRdRr, 16);
  case BLUCPU::AtomicLoadAnd8:
    return insertAtomicArithmeticOp(MI, MBB, BLUCPU::ANDRdRr, 8);
  case BLUCPU::AtomicLoadAnd16:
    return insertAtomicArithmeticOp(MI, MBB, BLUCPU::ANDWRdRr, 16);
  case BLUCPU::AtomicLoadOr8:
    return insertAtomicArithmeticOp(MI, MBB, BLUCPU::ORRdRr, 8);
  case BLUCPU::AtomicLoadOr16:
    return insertAtomicArithmeticOp(MI, MBB, BLUCPU::ORWRdRr, 16);
  case BLUCPU::AtomicLoadXor8:
    return insertAtomicArithmeticOp(MI, MBB, BLUCPU::EORRdRr, 8);
  case BLUCPU::AtomicLoadXor16:
    return insertAtomicArithmeticOp(MI, MBB, BLUCPU::EORWRdRr, 16);
  }

  assert((Opc == BLUCPU::Select16 || Opc == BLUCPU::Select8) &&
         "Unexpected instr type to insert");

  const BLUCPUInstrInfo &TII = (const BLUCPUInstrInfo &)*MI.getParent()
                                ->getParent()
                                ->getSubtarget()
                                .getInstrInfo();
  DebugLoc dl = MI.getDebugLoc();
  ...
  if (FallThrough != nullptr) {
    BuildMI(MBB, dl, TII.get(BLUCPU::RJMPk)).addMBB(FallThrough);
  }
  ...
  BLUCPUCC::CondCodes CC = (BLUCPUCC::CondCodes)MI.getOperand(3).getImm();
  BuildMI(MBB, dl, TII.getBrCond(CC)).addMBB(trueMBB);
  BuildMI(MBB, dl, TII.get(BLUCPU::RJMPk)).addMBB(falseMBB);
  MBB->addSuccessor(falseMBB);
  MBB->addSuccessor(trueMBB);

  // Unconditionally flow back to the true block
  BuildMI(falseMBB, dl, TII.get(BLUCPU::RJMPk)).addMBB(trueMBB);
  falseMBB->addSuccessor(trueMBB);

  // Set up the Phi node to determine where we came from
  BuildMI(*trueMBB, trueMBB->begin(), dl, TII.get(BLUCPU::PHI),
...
BLUCPUTargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
  ...
  if (Constraint.size() == 1) {
    ...
    case 'a': // Simple upper registers r16..r23.
      if (VT == MVT::i8)
        return std::make_pair(0U, &BLUCPU::LD8loRegClass);
      else if (VT == MVT::i16)
        return std::make_pair(0U, &BLUCPU::DREGSLD8loRegClass);
      break;
    case 'b': // Base pointer registers: y, z.
      if (VT == MVT::i8 || VT == MVT::i16)
        return std::make_pair(0U, &BLUCPU::PTRDISPREGSRegClass);
      break;
    case 'd': // Upper registers r16..r31.
      if (VT == MVT::i8)
        return std::make_pair(0U, &BLUCPU::LD8RegClass);
      else if (VT == MVT::i16)
        return std::make_pair(0U, &BLUCPU::DLDREGSRegClass);
      break;
    case 'l': // Lower registers r0..r15.
      if (VT == MVT::i8)
        return std::make_pair(0U, &BLUCPU::GPR8loRegClass);
      else if (VT == MVT::i16)
        return std::make_pair(0U, &BLUCPU::DREGSloRegClass);
      break;
    case 'e': // Pointer register pairs: x, y, z.
      if (VT == MVT::i8 || VT == MVT::i16)
        return std::make_pair(0U, &BLUCPU::PTRREGSRegClass);
      break;
    case 'q': // Stack pointer register: SPH:SPL.
      return std::make_pair(0U, &BLUCPU::GPRSPRegClass);
    case 'r': // Any register: r0..r31.
      if (VT == MVT::i8)
        return std::make_pair(0U, &BLUCPU::GPR8RegClass);
      else if (VT == MVT::i16)
        return std::make_pair(0U, &BLUCPU::DREGSRegClass);
      break;
    ...
    case 'w': // Special upper register pairs: r24, r26, r28, r30.
      if (VT == MVT::i8 || VT == MVT::i16)
        return std::make_pair(0U, &BLUCPU::IWREGSRegClass);
      break;
    case 'x': // Pointer register pair X: r27:r26.
    case 'X':
      if (VT == MVT::i8 || VT == MVT::i16)
        return std::make_pair(unsigned(BLUCPU::R27R26), &BLUCPU::PTRREGSRegClass);
      break;
    case 'y': // Pointer register pair Y: r29:r28.
    case 'Y':
      if (VT == MVT::i8 || VT == MVT::i16)
        return std::make_pair(unsigned(BLUCPU::R29R28), &BLUCPU::PTRREGSRegClass);
      break;
    case 'z': // Pointer register pair Z: r31:r30.
    case 'Z':
      if (VT == MVT::i8 || VT == MVT::i16)
        return std::make_pair(unsigned(BLUCPU::R31R30), &BLUCPU::PTRREGSRegClass);
      break;
    default:
      break;
    }
