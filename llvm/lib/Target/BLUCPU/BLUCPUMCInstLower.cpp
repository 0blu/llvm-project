...
MCOperand
BLUCPUMCInstLower::lowerSymbolOperand(const MachineOperand &MO, MCSymbol *Sym,
                                   const BLUCPUSubtarget &Subtarget) const {
  unsigned char TF = MO.getTargetFlags();
  const MCExpr *Expr = MCSymbolRefExpr::create(Sym, Ctx);

  bool IsNegated = false;
  if (TF & BLUCPUII::MO_NEG) {
    IsNegated = true;
  }
...
void BLUCPUMCInstLower::lowerInstruction(const MachineInstr &MI,
                                      MCInst &OutMI) const {
  auto &Subtarget = MI.getParent()->getParent()->getSubtarget<BLUCPUSubtarget>();
